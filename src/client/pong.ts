...
const sketch = (p5: P5) => {
  ...
  p5.draw = () => {
    ...
    // player paddles
    p5.fill("white");
    p5.rect(player1.x, player1.y, player1.width, player1.height);
    p5.rect(player2.x, player2.y, player2.width, player2.height);

    // pong ball
    p5.fill("red");
    p5.ellipse(ball.x, ball.y, 10, 10);
    ...
    const yPaddle = 4;
    // 'a' key
    if (p5.keyIsDown(65) && player1.y > 5) {
      player1.y -= yPaddle;
    }
    // 'z' key
    if (p5.keyIsDown(90) && player1.y < screenHeight - player1.height) {
      player1.y += yPaddle;
    }

    // player 2 paddle movement
    if (p5.keyIsDown(p5.UP_ARROW) && player2.y > 5) {
      player2.y -= yPaddle;
    }
    if (p5.keyIsDown(p5.DOWN_ARROW) && player2.y < screenHeight - player1.height) {
      player2.y += yPaddle;
    }
